...
# v1.5 Fix OTA message typo, added arguments "debug" (enable debug mode on the fly) 
#      and "nochangelog" (don't upload changelogs) to pass these selections in.  Separated
#      logging output into relevant parts. 
# v1.6 Reversed order of upload for changelog and zip file so bot sees zip last.
# v1.7 Updated for ten
# v1.8 Updated to pick up ROM base folder name for compiling FTP dir
#
VERSION="v1.8"
...
SFUSER="bhi768"	# SourceForge username
SFSSHKEY="/home/bhi/.ssh/id_rsa" # set to your local private SSH key path
FTPDIR="out/target/product"	# local top-level ftp upload directory
UCL=true # set this to upload the changelog with the zip file (default behavior)
...
ARG1=$1
ARG2=$2
if [[ $ARG1 == $NOCL || $ARG2 == $NOCL ]]; then
    UCL=false
    echo -e "Found 'nochangelog' flag.  Changelogs not uploading for this run."

fi
if [[ $ARG1 == $DBG || $ARG2 == $DBG ]]; then
    DEBUG=true
    echo -e "Found 'debug' flag.  DEBUG mode is enabled for this run."
fi
#
...
#------------------- Core Upload Code -----------------#
echo -e "\nInitiating SFTP connection to ${UHOST} with user ${SFUSER}..."
tDev=${#DEVICES[@]}
